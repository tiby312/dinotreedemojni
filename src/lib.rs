...
fn rust_game_step(
    ...
    let radius: &mut f32 = { radius };

    let num_verticies: &mut i32 = { num_verticies };
...
pub mod android {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn Java_kenreed_dinotreedemo_DinoGame_gameCreate(
        env: JNIEnv,
        _: JClass,
        startx: jint,
        starty: jint,
        ...
        env.set_float_array_region(radius, 0, &mradius).unwrap();
    ...
    #[no_mangle]
    pub unsafe extern "C" fn Java_kenreed_dinotreedemo_DinoGame_gameDestroy(
        _env: JNIEnv,
        _: JClass,
        game: jlong,
    ) {
        let game = conv::into_pointer(game);
        rust_game_destroy(&mut *game);
    }

    #[no_mangle]
    pub unsafe extern "C" fn Java_kenreed_dinotreedemo_DinoGame_updateVerticies(
        env: JNIEnv,
        _: JClass,
        game: jlong,
        jverts: JByteBuffer,
    ) {
        let game = &mut *conv::into_pointer(game);

        let verts: &mut [f32] = {
            let (buffptr, bufflen) = {
                let buff: &mut [u8] = env.get_direct_buffer_address(jverts).unwrap();
                let k: ReprMut<u8> = std::mem::transmute(buff);
                (k.ptr, k.size)
            };
    ...
    pub unsafe extern "C" fn Java_kenreed_dinotreedemo_DinoGame_gameStep(
        ...
        let is_game = rust_game_step(
            ...
            &mut mborder,
            &mut mradius[0],
            &mut mnum_verticies[0],
        );

        env.set_float_array_region(colors, 0, &mcolor).unwrap();
        env.set_float_array_region(new_border, 0, &mborder).unwrap();
        env.set_float_array_region(radius, 0, &mradius).unwrap();
    ...
    mod conv {
        use super::*;
        pub unsafe fn into_jlong(a: *mut MenuGameJni) -> jlong {
            let a: u32 = unsafe { std::mem::transmute(a) };

            a as jlong
        }
        pub unsafe fn into_pointer(a: jlong) -> *mut MenuGameJni {
